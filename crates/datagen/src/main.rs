...
use madrid_cita_previa::{AppointmentSession, DataGenModel, DataGenOffice, DataGenOfficeProcedure, DataGenProcedure, NetOfficeBasicInfoModel, NetOfficeModel, NetProcedureModel, OfficeId, ProcedureId};
...
pub fn filter_relevant_offices(offices: Vec<NetOfficeBasicInfoModel>) -> Vec<NetOfficeBasicInfoModel> {
    offices
...
async fn main0() -> anyhow::Result<()> {
    ...
    let model = DataGenModel {
        offices: office_data.into_iter().map(|(office_basic, office)| {
        DataGenOffice {
            name: office.name,
            group: office_basic.group,
            id: office_basic.id,
            procedures: office.procedures.into_iter().map(|proc| {
                DataGenOfficeProcedure {
        ...
        procedures: procs.into_iter().map(|proc| {
            DataGenProcedure {
    ...
    File::create("model2.json").await.unwrap().write_all(&str.as_bytes()).await.unwrap();
    Ok(())
}
